# Terraform configuration for LLM Tab Cleaner infrastructure
...
terraform {
  ...
  backend "s3" {
    bucket         = "llm-tab-cleaner-terraform-state"
    key            = "infrastructure/terraform.tfstate"
    region         = "us-west-2"
    encrypt        = true
    dynamodb_table = "llm-tab-cleaner-terraform-lock"
  }
}
...
resource "aws_key_pair" "eks_nodes" {
  ...
  tags = {
    Name = "${var.cluster_name}-nodes-key"
  }
}
...
resource "aws_elasticache_replication_group" "main" {
  ...
  description                = "Redis cluster for LLM Tab Cleaner"
...
resource "aws_s3_bucket" "data" {
  bucket = "${var.cluster_name}-data-${random_string.bucket_suffix.result}"
